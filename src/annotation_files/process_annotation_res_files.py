...
class support_sen_files:
    ...
    #         for claim_num in ['36','39','40','41','42','45','46','47','49','50','53','54','55','58','59','60','61','62','66','69','70']:#,'39']:#,]:#,'6','7','8','14','15']: #batch 1#             for claim_num in ['14','15']:
        ...
        #30.06 add the crowdflower results processing
            ...
            output_path=r"C:\study\technion\MSc\Thesis\Y!\support_test\output_crowdflower_second_trial"
            ...
            #25.02.15 addition - want to check the stats for the wikipedia sentences only
            #identify the wiki sen using another dict
            claim_sen_relevance_dict_wiki = utils_linux.read_pickle("claim_sen_relevance_dict_wiki")
            claims_dict = utils_linux.read_pickle("claim_dict")
            check="reduced"
            ...
            relevance_mat= np.zeros((1,2)) # 14/09/14 update -  measure the agreement only between non relevant and relevant sentences
            for filename in os.listdir(output_path):
                curr_support_mat=np.zeros((30, len(answers_possibilities))) # 10 sentences, 4 rating, was 3 rating for batch 1 and 2 
                curr_relevance_mat = np.zeros((30, 2)) #first entry is for not relevant sen, second if for relevant (support, contra, neut...)
                
                if filename.split("_")[0]=="f":
                    claim_num=filename.split("_")[1].split(".")[0]
                    claim_text = claims_dict[claim_num]
